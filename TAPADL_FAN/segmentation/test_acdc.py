...
def main():
    ...
    for noise_name in name_list:
        tmp_show_dir = None
        if args.show_dir:
            tmp_show_dir = os.path.join(args.show_dir, noise_name)
            os.makedirs(tmp_show_dir, exist_ok=True)
        ...
        test_dir = os.path.join(test_dir, corrupt_type + '/test')
        # test_dir = test_dir + corrupt_type + '/' + str(severity)
        cfg.data.test.img_dir = test_dir
        cfg.data.test.ann_dir = os.path.join(cfg.data.test.ann_dir, corrupt_type + '/test')
        ...
        if not distributed:
            ...
            outputs = single_gpu_test(model, data_loader, args.show, tmp_show_dir,
                                    efficient_test)
        else:
        ...
        if args.eval:
            ...
            average_acc[noise_name] = res_sum
            np.savetxt(results_file + '_' + noise_name + '_' + '%.2f' % (res_sum) + '.csv', results)
            print('average score is:', res_sum )
