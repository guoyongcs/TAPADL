...
class MaskTokenMixing(nn.Module):
    # attention with cross-token projector
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
                 sr_ratio=1, linear=False, share_atten=False, drop_path=0., emlp=False, sharpen_attn=False,
                 mlp_hidden_dim=None, act_layer=nn.GELU, drop=None, norm_layer=nn.LayerNorm):
        super().__init__()
        ...
        # q
        K = 3
        M = math.floor((3 * dim) / (K*K + 3))
        self.qkv = nn.Sequential(
            nn.Conv2d(dim, M, K, 1, (K-1)//2, bias=qkv_bias),
            nn.ReLU(inplace=True),
        )
        self.fc = nn.Linear(M, dim*3, bias=qkv_bias)
    ...
    def forward(self, x, H, W, atten=None, return_attention=False):
        ...
        qkv = self.qkv(x)
        qkv = qkv.permute(0, 2, 3, 1).reshape(B, N, -1)
        qkv = self.fc(qkv)
...
class FANBlock(nn.Module):
    ...
    def forward(self, x, attn=None, return_attention=False):
        ...
        if self.training and B>=2:
            target = x[B//2:]
            self.recon_loss = F.mse_loss(x[0:B//2], target)
...
class MaskFANBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., sharpen_attn=False,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, eta=1., sr_ratio=1., downsample=None, c_head_num=None):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = MaskTokenMixing(dim, num_heads=num_heads, qkv_bias=qkv_bias, mlp_hidden_dim=int(dim * mlp_ratio), sharpen_attn=sharpen_attn,
                                attn_drop=attn_drop, proj_drop=drop, drop=drop, drop_path=drop_path, sr_ratio=sr_ratio)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
...
class FAN(nn.Module):
    ...
    def forward(self, x, x1=None):
        if x1 is not None:
            ...
            x = self.head(x)
            return x[0:x.size(0)//2], x[x.size(0)//2:]
        else:
            x = self.forward_features(x)
            x = self.head(x)
            return x
...
def _create_afat(variant, pretrained=False, default_cfg=None, **kwargs):
    default_cfg = default_cfg or default_cfgs[variant]
    model = build_model_with_cfg(
        AFAT, variant, pretrained, default_cfg=default_cfg, pretrained_filter_fn=checkpoint_filter_fn, **kwargs)
    return model
