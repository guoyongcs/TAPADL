...
from .convnext_utils import _create_hybrid_backbone, DilationPredictor, DilatedAvgPooling
...
class Mlp(nn.Module):
    ...
    def flops(self, N):
        ...
        # self.dwconv
        flops += 9 * self.hidden_features * N
        # x = self.fc2(x)
...
class ClassAttentionBlock(nn.Module):
    ...
    def flops(self):
        flops = 0
        # norm1
        flops += self.dim * self.N
        # MSA
        flops += self.attn.flops(self.N+1)
        # mlp
        flops += self.in_features * self.hidden_features * 2
        # norm2
        flops += self.dim * self.N
        return flops
...
class TAPFANBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., sharpen_attn=False,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, eta=1., sr_ratio=1., downsample=None, c_head_num=None, num_scales=1):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x, attn=None, remove_head=None):
        x, attn_list = x
        H, W = self.H, self.W

        B, N, C = x.shape
        if remove_head is not None:
            x = x.reshape(B, N, self.attn.num_heads, C // self.attn.num_heads)
            x[:,:,remove_head,:] = x[:,:,remove_head,:] * 0
            x = x.reshape(B, N, C)
        ...
        x_new, attn_s = self.attn(self.norm1(x_attn), H, W)
        x = x + self.drop_path(self.gamma1 * x_new)
    ...
    def flops(self):
        ...
        H, W = self.H, self.W
        # DilationPredictor
        flops += self.dim * self.num_scales * H * W * 9 + self.num_scales * self.num_scales * H * W * 9
        # norm1
        flops += self.dim * H * W
        # MSA
...
class FAN(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12, sharpen_attn=False, channel_dims=None,
                 ...
                 act_layer=None, norm_layer=None,se_mlp=False, cls_attn_layers=2, use_pos_embed=True, eta=1., tokens_norm=False, c_head_num=None, hybrid_patch_size=2, head_init_scale=1.0, num_scales=1):
        ...
        for i in range(depth):
            ...
            self.blocks.append( 
                build_block(
                    ...
                    downsample=downsample, c_head_num=c_head_num[i] if c_head_num is not None else None, num_scales=num_scales))
    ...
    def forward_features(self, x, remove_head=None):
        B = x.shape[0]
        ...
        for blk in self.blocks:
            ...
            if self.use_checkpoint:
                ...
                if blk_id == len(self.blocks)-1 and not self.training:
                    x = blk(x, remove_head=remove_head)
                else:
                    x = blk(x)
            H, W = blk.H, blk.W
            x, attn_list = x
    ...
    def forward(self, x, return_attn=False, remove_head=None):
        x, attn_list = self.forward_features(x, remove_head)
        x = self.head(x)
        if return_attn:
            return x, attn_list
        else:
    ...
    def flops(self):
        flops = 0
        flops += compute_flops(self.patch_embed, (1, 3, 224, 224))
        ...
        for blk in self.cls_attn_blocks:
            flops += blk.flops()
        # norm
        flops += self.num_features * self.cls_attn_blocks[-1].N
        # head
...
def _create_fan(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    model = build_model_with_cfg(
        FAN, variant, pretrained, pretrained_filter_fn=checkpoint_filter_fn, **kwargs)
    return model


# TAP-FAN-ViT Models
@register_model
def tap_fan_tiny_12_p16_224(pretrained=False, bn_tf=False, **kwargs):
    depth = 12
...
@register_model
def tap_fan_small_12_p16_224_se_attn(pretrained=False, **kwargs):
    depth = 12
    sr_ratio = [1] * (depth//2) + [1] * (depth//2)
    model_kwargs = dict(
...
@register_model
def tap_fan_small_12_p16_224(pretrained=False, bn_tf=False, **kwargs):
    depth = 12
    sr_ratio = [1] * depth
    model_kwargs = dict(
...
@register_model
def tap_fan_base_18_p16_224(pretrained=False, **kwargs):
    depth = 18
    sr_ratio = [1] * (depth//2) + [1] * (depth//2)
    model_kwargs = dict(
...
@register_model
def tap_fan_large_24_p16_224(pretrained=False, **kwargs):
    depth = 24
    sr_ratio = [1] * (depth//2) + [1] * (depth//2)
    model_kwargs = dict(
...
@register_model
def tap_fan_tiny_8_p4_hybrid(pretrained=False, **kwargs):
    depth = 8
...
@register_model
def tap_fan_small_12_p4_hybrid(pretrained=False, **kwargs):
    depth = 10
...
@register_model
def tap_fan_base_16_p4_hybrid(pretrained=False, **kwargs):
    depth = 16
    sr_ratio = [1] * (depth//2) + [1] * (depth//2)
    model_args = dict(depths=[3, 3], dims=[128, 256, 512, 1024], use_head=False)
    backbone = _create_hybrid_backbone(pretrained=False, pretrained_strict=False, **model_args)
...
@register_model
def tap_fan_large_16_p4_hybrid(pretrained=False, **kwargs):
    depth = 22
    sr_ratio = [1] * (depth//2) + [1] * (depth//2)
...
@register_model
def tap_fan_Xlarge_16_p4_hybrid(pretrained=False, **kwargs):
    """
...
@register_model
def tap_fan_swin_tiny_patch4_window7_224(pretrained=False, **kwargs):
...
# TAP-FAN-Swin Models
@register_model
def tap_fan_swin_small_patch4_window7_224(pretrained=False, **kwargs):
...
@register_model
def tap_fan_swin_base_patch4_window7_224(pretrained=False, **kwargs):
    """ Swin-S @ 224x224, trained ImageNet-1k
    """
    mlp_type = ['FAN', 'FAN', 'FAN', 'Mlp']
    model_kwargs = dict(
...
@register_model
def tap_fan_swin_large_patch4_window7_224(pretrained=False, **kwargs):
    """ Swin-S @ 224x224, trained ImageNet-1k
    """
    mlp_type = ['FAN', 'FAN', 'FAN', 'Mlp']
    model_kwargs = dict(
