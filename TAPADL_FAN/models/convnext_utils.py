...
class ConvNeXt(nn.Module):
    ...
    def __init__(
            ...
            head_init_scale=1., head_norm_first=False, norm_layer=None, drop_rate=0., drop_path_rate=0., remove_last_downsample=False, num_scales=None,
    ):
        super().__init__()
        ...
        self.token_pools, self.dilation_predictor = None, None
        if num_scales is not None:
            self.token_pools = nn.ModuleList([])
            for i in range(num_scales):
                self.token_pools.append(DilatedAvgPooling(i))
            self.dilation_predictor = DilationPredictor(dims[0], len(self.token_pools))
    ...
    def forward_features(self, x):
        ...
        if self.dilation_predictor is not None:
            B, C, _, _ = x.shape
            mask = self.dilation_predictor(x) # B 1 H W K
            self.pred_scale = mask
            mixed_dilations = []
            for i in range(len(self.token_pools)):
                mixed_dilations.append(self.token_pools[i](x))
            x = torch.cat(mixed_dilations, dim=-1)
