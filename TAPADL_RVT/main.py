...
def get_args_parser():
    parser = argparse.ArgumentParser('TAPADL training and evaluation script', add_help=False)
    ...
    parser.add_argument('--data-set', default='IMNET', choices=['CIFAR10', 'CIFAR100', 'IMNET', 'IMNET100', 'INAT', 'INAT19'],
    ...
    # TAP-ADL Params
    parser.add_argument('--num-scales', default=4, type=int, help="number of scale levels for TAP")
    parser.add_argument('--threshold', default=2, type=int, help="threshold for attention score")
    parser.add_argument('--adloss', type=float, default=1, help='weight for attention diversification loss')
...
def main(args):
    ...
    logger = logging.getLogger('ADViT')
    ...
    if not args.eval and os.path.exists(checkpoint_path):
    ...
    if dist_rank == 0:
        saver = utils.MyCheckpointSaver(
            ...
            checkpoint_dir=args.output_dir, recovery_dir=args.output_dir, decreasing=False, max_history=5)
        if args.resume:
            try:
    ...
    if args.eval:
        ...
        test_stats = evaluate(logger, data_loader_val, model, device, args=args)
        clean_acc = math.ceil(test_stats['acc1'] * 10) / 10  # keep one decimal place
        logger.info(f"Accuracy of the network on the {len(dataset_val)} test images: {clean_acc:.1f}%")

        if args.inc_path:
            _ = eval_inc(logger, model, device, args=args)
        ...
        if args.inr_path:
            ...
            indices_in_1k = [i for i in range(1000) if imagenet_r_mask[i] is True]
            ...
            test_stats = evaluate(logger, inr_data_loader, model, device, args=args, indices_in_1k=indices_in_1k)
        ...
        if args.insk_path:
            ...
            test_stats = evaluate(logger, insk_data_loader, model, device, args=args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if model_ema is not None and not args.model_ema_force_cpu:
            ema_eval_metrics = evaluate(logger, data_loader_val, model_ema.module, device, args=args)
        ...
        if args.output_dir and utils.is_main_process():
            ...
            if corruption_accuracy>0:
                writer.add_scalar('cACC', corruption_accuracy, epoch)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser('TAPADL training and evaluation script', parents=[get_args_parser()])
